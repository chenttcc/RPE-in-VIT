...
import numpy as np
from .regular_attention import NormalAttention, GeneralAttention, PolynomialAttention, ExponentialAttention
...
class VisionTransformer(Model):
    def __init__(self, image_size, patch_size, num_channels, num_classes, embed_dim, depth, num_heads, mlp_ratio, drop_rate, rpe_method, multihead):
        ...
        self.pos_embed = self.add_weight(name="pos_embed", shape=(1, 1 + self.num_patches, embed_dim), initializer="random_normal")
        self.dropout = layers.Dropout(drop_rate)

        self.transformer_blocks = [TransformerBlock(image_size / patch_size, embed_dim, num_heads, mlp_ratio, drop_rate, rpe_method, multihead) for _ in range(depth)]
...
class TransformerBlock(layers.Layer):
    def __init__(self, lengthofside, dim, num_heads, mlp_ratio, drop_rate, rpe_method, multihead, drop_path_rate=0.1):
        super(TransformerBlock, self).__init__()
        if rpe_method == 'no_rpe':
            self.attention = NormalAttention(dim, num_heads, attn_drop=drop_rate, proj_drop=drop_rate)
        elif rpe_method == 'general_rpe':
            self.attention = GeneralAttention(lengthofside, dim, num_heads, attn_drop=drop_rate, proj_drop=drop_rate, multihead = multihead)
        elif rpe_method == 'exponential_rpe':
            self.attention = ExponentialAttention(lengthofside, dim, num_heads, attn_drop=drop_rate, proj_drop=drop_rate, multihead = multihead)
        elif rpe_method == 'polynomial_rpe':
            self.attention = PolynomialAttention(lengthofside, dim, num_heads, attn_drop=drop_rate, proj_drop=drop_rate, multihead = multihead)
        else:
            raise Exception('Rpe method is wrong')
